#!/usr/bin/env node
/** RC10.6 update API verification + canonical APK SHA256 match. */
import { createHash } from "node:crypto";
import { readFileSync, writeFileSync, mkdirSync, existsSync } from "node:fs";
import { resolve } from "node:path";

const STAGING = process.env.STAGING_BASE_URL ?? "https://web-production-e56fb.up.railway.app";
const OUT = resolve("artifacts/closed-beta-rc10.6/update-matrix.json");
const UPDATE_API_OUT = resolve("artifacts/closed-beta-rc10.6/update-api-verification.json");
const manifestPath = resolve("artifacts/closed-beta-rc10.6/build-manifest.json");
const TARGET_CODE = 22;
...
async function main() {
  const probes = [];
  for (const code of [17, 18, 19, 20, 21, 22, 23]) {
  ...
  const mrpPublished = probes.some((p) => p.installedVersionCode === 21 && p.verdict === "PASS");
  const report = {
    ...
    matrix: [
      { from: "code 17", to: "RC10.6 (code 22)", expected: "OPTIONAL_UPDATE", physical: "NOT_RUN" },
      { from: "code 18", to: "RC10.6 (code 22)", expected: "OPTIONAL_UPDATE", physical: "NOT_RUN" },
      { from: "code 19", to: "RC10.6 (code 22)", expected: "OPTIONAL_UPDATE", physical: "NOT_RUN" },
      { from: "code 20", to: "RC10.6 (code 22)", expected: "OPTIONAL_UPDATE", physical: "NOT_RUN" },
      { from: "RC10.5 (code 21)", to: "RC10.6 (code 22)", expected: "OPTIONAL_UPDATE", physical: "NOT_RUN" },
      { from: "RC10.6 (code 22)", to: "RC10.6", expected: "NO_UPDATE", physical: "NOT_RUN" },
      { from: "future code 23+", to: "RC10.6", expected: "NO_DOWNGRADE", physical: "NOT_RUN" },
    ],
