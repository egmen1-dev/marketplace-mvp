#!/usr/bin/env node
/** Publish RC8 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc8/mrp-publish-report.json");

const manifestPath = resolve("artifacts/closed-beta-rc8/build-manifest.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC8 — EPIC 152 Seller Transaction Loop + EPIC 154 Critical Path",
    "Seller sales tabs: Новые → Принять → В работе → Завершенные",
    "Buyer order detail with human-readable status timeline",
    "Checkout return deep link lot://order/{id} + success banner",
    "Duplicate order protection (advisory lock + submit guards)",
    "Seller storefront trust block on PDP",
    "Upgrade path: RC3–RC7 (code 7-12) → RC8 (code 13)",
  ].join("\n"),
...
async function main() {
  ...
  const report = {
    ...
    status,
    updatePath: {
      code7: "OPTIONAL_UPDATE→13",
      code8: "OPTIONAL_UPDATE→13",
      code9: "OPTIONAL_UPDATE→13",
      code10: "OPTIONAL_UPDATE→13",
      code11: "OPTIONAL_UPDATE→13",
      code12: "OPTIONAL_UPDATE→13",
      code13: "NO_UPDATE",
      code14: "NO_UPDATE",
    },
