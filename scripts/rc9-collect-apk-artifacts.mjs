#!/usr/bin/env node
/** Collect APK artifacts for RC9 build (sha256, aapt, apkanalyzer, build-manifest). */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const ARTIFACT_DIR = resolve("artifacts/closed-beta-rc9");
const APK_NAME = "lot_android_closed_beta_0.1.14_beta.1.apk";
...
const manifest = {
  ...
  artifact: {
    fileName: APK_NAME,
    path: `artifacts/closed-beta-rc9/${APK_NAME}`,
    downloadUrl: `https://raw.githubusercontent.com/egmen1-dev/marketplace-mvp/main/artifacts/closed-beta-rc9/${APK_NAME}`,
    sha256,
    sizeBytes,
  },
  android: {
  ...
  embeddedMetadata: {
    epic157_uxPolish: true,
    epic158_sellerLotCreation: true,
    productImageFallback: true,
    productCartCta: true,
    sellCreateRoute: true,
    sellerLotsTabs: true,
  ...
  buildNotes: {
    release: "RC9 — EPIC 157 UX Polish + EPIC 158 Seller LOT Creation",
    previousRelease: "RC8 0.1.13-beta.1 (code 13)",
  },
};
