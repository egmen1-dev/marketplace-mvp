#!/usr/bin/env node
/** RC10 staging runtime gate — seller LOT APIs + EPIC 158/159 parity. */
import { mkdirSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const STAGING = process.env.STAGING_BASE_URL ?? "https://web-production-e56fb.up.railway.app";
const OUT = resolve("artifacts/closed-beta-rc10/backend-parity.json");
...
async function main() {
  ...
  const buyerToken = await login(BUYER, "rc10-runtime-buyer");
  const sellerToken = await login(SELLER, "rc10-runtime-seller");
