#!/usr/bin/env node
/** Publish RC6 to MRP via staging admin API. */
...
const manifestPath = resolve("artifacts/closed-beta-rc6/build-manifest.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC6 — CommerceHeader + buyer↔seller chat",
    "Web→mobile parity audit (PR #146)",
    "Messages inbox + conversation threads",
    "Entry points: PDP, seller storefront, orders, profile, header",
    "Unread badges from real API (no polling)",
    "Upgrade path: RC3–RC5.1 (code 7-10) → RC6 (code 11)",
  ].join("\n"),
