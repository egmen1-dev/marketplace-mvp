#!/usr/bin/env node
/** RC10.2 APK forensics — publish truth hotfix symbols in assembled bundle. */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const apkPath = process.argv[2] ?? "artifacts/closed-beta-rc10.2/lot_android_closed_beta_0.1.15_beta.3.apk";
const OUT = resolve("artifacts/closed-beta-rc10.2/apk-feature-verification.json");
...
const symbolGroups = {
  publishTruth: [
    ...
    { name: "PUBLISHED", pattern: "PUBLISHED" },
    { name: "PENDING_REVIEW", pattern: "PENDING_REVIEW" },
    { name: "publishOutcome", pattern: "publishOutcome" },
    { name: "pendingReviewTitle", pattern: "pendingReviewTitle" },
    { name: "sellerLotDetailRoute", pattern: "sellerLotDetailRoute" },
    { name: "sellerLotsTabForOutcome", pattern: "sellerLotsTabForOutcome" },
    { name: "publishReviewLabel", pattern: "publishReviewLabel" },
    { name: "fetchSellerLot", pattern: "fetchSellerLot" },
    { name: "mapMutationOutcome", pattern: "mapMutationOutcome" },
  ],
  photoUpload: [
...
const legacyBrokenPatterns = [
  { name: "savedForReviewFallback", pattern: "reviewNote = LOT_CREATE_COPY.savedForReview" },
  { name: "successFromReviewNote", pattern: "setInfo(reviewNote)" },
];
...
const identityChecks = [
  { name: "versionName", ok: identity.versionName === "0.1.15-beta.3", expected: "0.1.15-beta.3", actual: identity.versionName },
  { name: "versionCode", ok: identity.versionCode === "18", expected: "18", actual: identity.versionCode },
  ...
  { name: "rc10_2Label", ok: bundleContains("RC10.2"), expected: "RC10.2 in bundle", actual: bundleContains("RC10.2") ? "present" : "missing" },
