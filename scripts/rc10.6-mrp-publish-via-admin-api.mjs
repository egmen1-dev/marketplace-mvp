#!/usr/bin/env node
/** Publish RC10.6 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc10.6/mrp-publish-report.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC10.6 — physical retest build (PR #198)",
    "Fix: photo Continue one-tap + truthful upload readiness state",
    "Fix: submit black-hole — explicit outcome, no silent CHARACTERISTICS_REQUIRED",
    "Fix: seller journey diagnostics ring buffer + copy surface",
    "Backend: async OCR off submit critical path (deployed)",
    "Upgrade path: codes 17–21 → RC10.6 (code 22)",
  ].join("\n"),
...
async function main() {
  ...
  const report = {
    ...
    status,
    updatePath: {
      code17: "OPTIONAL_UPDATE→22",
      code18: "OPTIONAL_UPDATE→22",
      code19: "OPTIONAL_UPDATE→22",
      code20: "OPTIONAL_UPDATE→22",
      code21: "OPTIONAL_UPDATE→22",
      code22: "NO_UPDATE",
      code23plus: "NO_DOWNGRADE",
    },
