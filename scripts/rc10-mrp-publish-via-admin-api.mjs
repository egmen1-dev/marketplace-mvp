#!/usr/bin/env node
/** Publish RC10 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc10/mrp-publish-report.json");

const manifestPath = resolve("artifacts/closed-beta-rc10/build-manifest.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC10 — EPIC 158.1–158.3 + EPIC 159 Seller Beta Acceptance",
    "Seller LOT creation: autosave, restore prompt, preview, publish",
    "Seller UX: simplified copy, orange CTA, human errors, sticky footer",
    "Preview: «Проверьте ЛОТ», cover photo, publish/save/back CTA order",
    "Update flow: foreground refresh, profile badge, modal copy",
    "Critical fix: save→publish updates same LOT (no duplicates)",
    "Upgrade path: RC3–RC9.1 (code 7-15) → RC10 (code 16)",
  ].join("\n"),
...
async function main() {
  ...
  const report = {
    ...
    status,
    updatePath: {
      code13: "OPTIONAL_UPDATE→16",
      code14: "OPTIONAL_UPDATE→16",
      code15: "OPTIONAL_UPDATE→16",
      code16: "NO_UPDATE",
    },
