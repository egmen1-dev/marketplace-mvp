#!/usr/bin/env node
/** Collect APK artifacts for RC9.1 build (sha256, aapt, apkanalyzer, build-manifest). */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const ARTIFACT_DIR = resolve("artifacts/closed-beta-rc9.1");
const APK_NAME = "lot_android_closed_beta_0.1.14_beta.2.apk";
...
const manifest = {
  ...
  artifact: {
    fileName: APK_NAME,
    path: `artifacts/closed-beta-rc9.1/${APK_NAME}`,
    downloadUrl: `https://raw.githubusercontent.com/egmen1-dev/marketplace-mvp/main/artifacts/closed-beta-rc9.1/${APK_NAME}`,
    sha256,
    sizeBytes,
  },
  android: {
  ...
  buildNotes: {
    release: "RC9.1 — EPIC 158.1 Seller LOT UX Hardening",
    previousRelease: "RC9 0.1.14-beta.1 (code 14)",
  },
};
