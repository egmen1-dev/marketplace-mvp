#!/usr/bin/env node
/** Publish RC10.9 target to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc10.9/mrp-publish-report.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC10.9 — Self-Update V2 physical proof target",
    "Carry-forward: Android Self-Update V2 (chunked SHA, state machine, diagnostics)",
    "Physical proof path: RC10.8 bootstrap → in-app update → RC10.9",
    "Upgrade path: codes 17–24 → RC10.9 (code 25)",
  ].join("\n"),
