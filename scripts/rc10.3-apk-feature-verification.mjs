#!/usr/bin/env node
/** RC10.3 APK forensics — P0.1 dynamic characteristics + publish truth symbols. */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const apkPath = process.argv[2] ?? "artifacts/closed-beta-rc10.3/lot_android_closed_beta_0.1.15_beta.4.apk";
const OUT = resolve("artifacts/closed-beta-rc10.3/apk-feature-verification.json");
...
const symbolGroups = {
  dynamicCharacteristics: [
    ...
    { name: "characteristicsTitle", pattern: "characteristicsTitle" },
    { name: "humanCharacteristicPrompt", pattern: "humanCharacteristicPrompt" },
    { name: "handleCharacteristicRejection", pattern: "handleCharacteristicRejection" },
    { name: "addCharacteristics", pattern: "addCharacteristics" },
  ],
  publishTruth: [
...
const legacyBrokenPatterns = [
  { name: "emptyCharacteristicsAlways", pattern: "characteristics: []" },
...
const identityChecks = [
  { name: "versionName", ok: identity.versionName === "0.1.15-beta.4", expected: "0.1.15-beta.4", actual: identity.versionName },
  { name: "versionCode", ok: identity.versionCode === "19", expected: "19", actual: identity.versionCode },
  ...
  { name: "rc10_3Label", ok: bundleContains("RC10.3"), expected: "RC10.3 in bundle", actual: bundleContains("RC10.3") ? "present" : "missing" },
