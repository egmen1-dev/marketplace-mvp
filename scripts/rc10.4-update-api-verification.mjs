#!/usr/bin/env node
/** RC10.4 update API verification + canonical APK SHA256 match. */
import { createHash } from "node:crypto";
import { readFileSync, writeFileSync, mkdirSync } from "node:fs";
import { resolve } from "node:path";

const STAGING = process.env.STAGING_BASE_URL ?? "https://web-production-e56fb.up.railway.app";
const OUT = resolve("artifacts/closed-beta-rc10.4/update-api-verification.json");
const manifest = JSON.parse(readFileSync(resolve("artifacts/closed-beta-rc10.4/build-manifest.json"), "utf8"));
...
async function probe(versionCode) {
  ...
  let verdict = "FAIL";
  if (versionCode <= 19) {
    verdict = updateState === "OPTIONAL_UPDATE" && targetCode === 20 ? "PASS" : "FAIL";
  } else if (versionCode === 20) {
...
async function main() {
  const probes = [];
  for (const code of [7, 13, 16, 17, 18, 19, 20, 21]) {
