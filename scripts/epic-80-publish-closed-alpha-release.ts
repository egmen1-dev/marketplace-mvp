#!/usr/bin/env tsx
/** EPIC-80 — publish Closed Alpha release 0.1.0-alpha to MRP registry */
import { ensureClosedAlphaReleasePublished } from "@/lib/mobile-release-platform/publish-closed-alpha";
import { CLOSED_ALPHA_RELEASE_010, CLOSED_ALPHA_APK_PREVIOUS_DOWNLOAD_URL } from "@/lib/mobile-release-platform/constants";

async function main() {
  const testerEmail = process.env.CLOSED_ALPHA_TESTER_EMAIL;
  const testerName = process.env.CLOSED_ALPHA_TESTER_NAME;

  console.log("Publishing Closed Alpha 0.1.0-alpha (legacy EPIC-80 script)…");
  console.log(`  version: ${CLOSED_ALPHA_RELEASE_010.versionName} (${CLOSED_ALPHA_RELEASE_010.versionCode})`);
  console.log(`  sha256:  ${CLOSED_ALPHA_RELEASE_010.sha256}`);
  console.log(`  url:     ${CLOSED_ALPHA_APK_PREVIOUS_DOWNLOAD_URL}`);

  const result = await ensureClosedAlphaReleasePublished({
    testerEmail,
    testerName,
    rolloutPercent: 100,
  });

  console.log("\nPublished:");
  console.log(JSON.stringify(result.release, null, 2));
  if (result.tester) {
    console.log(`\nTester assigned: ${result.tester.email}`);
  }
}

main().catch((err) => {
  console.error(err);
  process.exit(1);
});
