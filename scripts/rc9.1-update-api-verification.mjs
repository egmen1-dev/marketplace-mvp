#!/usr/bin/env node
/** RC9.1 update API verification + published APK SHA256 match. */
import { createHash } from "node:crypto";
import { readFileSync, writeFileSync, mkdirSync } from "node:fs";
import { resolve } from "node:path";

const STAGING = process.env.STAGING_BASE_URL ?? "https://web-production-e56fb.up.railway.app";
const OUT = resolve("artifacts/closed-beta-rc9.1/update-api-verification.json");
const manifest = JSON.parse(readFileSync(resolve("artifacts/closed-beta-rc9.1/build-manifest.json"), "utf8"));
...
async function probe(versionCode) {
  ...
  let verdict = "FAIL";
  if ([7, 8, 9, 10, 11, 12, 13, 14].includes(versionCode)) {
    verdict = updateState === "OPTIONAL_UPDATE" && targetCode === 15 ? "PASS" : "FAIL";
  } else if ([15, 16].includes(versionCode)) {
...
async function main() {
  const probes = [];
  for (const code of [7, 8, 9, 10, 11, 12, 13, 14, 15]) {
