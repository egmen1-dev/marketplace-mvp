#!/usr/bin/env node
/** RC10.2 update API verification + canonical APK SHA256 match. */
import { createHash } from "node:crypto";
import { readFileSync, writeFileSync, mkdirSync } from "node:fs";
import { resolve } from "node:path";

const STAGING = process.env.STAGING_BASE_URL ?? "https://web-production-e56fb.up.railway.app";
const OUT = resolve("artifacts/closed-beta-rc10.2/update-api-verification.json");
const manifest = JSON.parse(readFileSync(resolve("artifacts/closed-beta-rc10.2/build-manifest.json"), "utf8"));
...
async function probe(versionCode) {
  ...
  let verdict = "FAIL";
  if (versionCode <= 17) {
    verdict = updateState === "OPTIONAL_UPDATE" && targetCode === 18 ? "PASS" : "FAIL";
