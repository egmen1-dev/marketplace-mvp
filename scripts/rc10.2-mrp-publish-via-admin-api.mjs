#!/usr/bin/env node
/** Publish RC10.2 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc10.2/mrp-publish-report.json");

const manifestPath = resolve("artifacts/closed-beta-rc10.2/build-manifest.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC10.2 — P0 LOT publish state truth hotfix",
    "Fix: truthful publishOutcome (PUBLISHED / PENDING_REVIEW / SAVED)",
    "Fix: state-aware success screens and seller-owned LOT detail route",
    "Fix: My LOTs На проверке tab + non-public PDP Bearer auth",
    "Fix: CTA «Отправить на проверку» when moderation required",
    "Includes RC10.1 photo upload fix + RC10 seller flow stack",
    "Upgrade path: RC3–RC10.1 (code 7-17) → RC10.2 (code 18)",
  ].join("\n"),
