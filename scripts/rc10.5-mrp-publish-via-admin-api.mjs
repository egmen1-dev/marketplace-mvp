#!/usr/bin/env node
/** Publish RC10.5 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc10.5/mrp-publish-report.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC10.5 — My Lots tab consistency + create/preview P0 fixes",
    "Fix: canonical seller sections, request sequencing, server-backed search (#188)",
    "Fix: Preview validation contract, explicit blocker UX (#193)",
    "Upgrade path: codes 17–20 → RC10.5 (code 21)",
  ].join("\n"),
...
async function main() {
  ...
  const report = {
    ...
    status,
    updatePath: {
      code17: "OPTIONAL_UPDATE→21",
      code18: "OPTIONAL_UPDATE→21",
      code19: "OPTIONAL_UPDATE→21",
      code20: "OPTIONAL_UPDATE→21",
      code21: "NO_UPDATE",
      code22plus: "NO_UPDATE",
    },
