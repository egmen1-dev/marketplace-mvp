#!/usr/bin/env node
/** RC9.1 APK forensics — verify bundle contains EPIC 157/158 symbols (presence only, not physical PASS). */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const apkPath = process.argv[2] ?? "artifacts/closed-beta-rc9.1/lot_android_closed_beta_0.1.14_beta.2.apk";
const OUT = resolve("artifacts/closed-beta-rc9.1/apk-feature-verification.json");
...
const identityChecks = [
  { name: "versionName", ok: identity.versionName === "0.1.14-beta.2", expected: "0.1.14-beta.2", actual: identity.versionName },
  { name: "versionCode", ok: identity.versionCode === "15", expected: "15", actual: identity.versionCode },
  ...
  { name: "rc9Label", ok: bundleContains("RC9.1"), expected: "RC9.1 in bundle", actual: bundleContains("RC9.1") ? "present" : "missing" },
