#!/usr/bin/env node
/** Collect APK artifacts for RC10.2 build (P0 publish state truth hotfix). */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const ARTIFACT_DIR = resolve("artifacts/closed-beta-rc10.2");
const APK_NAME = "lot_android_closed_beta_0.1.15_beta.3.apk";
...
const manifest = {
  ...
  artifact: {
    fileName: APK_NAME,
    path: `artifacts/closed-beta-rc10.2/${APK_NAME}`,
    downloadUrl: `https://raw.githubusercontent.com/egmen1-dev/marketplace-mvp/main/artifacts/closed-beta-rc10.2/${APK_NAME}`,
    sha256,
    sizeBytes,
  },
  android: {
  ...
  embeddedMetadata: {
    ...
    resolveLotPublishOutcome: true,
    sellerLotDetailRoute: true,
    previousRelease: "RC10.1 0.1.15-beta.2 (code 17)",
  },
  buildNotes: {
    release: "RC10.2 — P0 LOT publish state truth + My LOTs visibility",
    previousRelease: "RC10.1 0.1.15-beta.2 (code 17)",
  },
};
