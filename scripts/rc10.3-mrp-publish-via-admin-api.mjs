#!/usr/bin/env node
/** Publish RC10.3 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc10.3/mrp-publish-report.json");

const manifestPath = resolve("artifacts/closed-beta-rc10.3/build-manifest.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC10.3 — P0.1 dynamic LOT category characteristics",
    "Fix: ProductType-aware characteristics form before preview",
    "Fix: required fields block preview with human copy (e.g. Укажите мощность)",
    "Fix: optional characteristics behind «Добавить характеристики»",
    "Fix: server CHARACTERISTICS_REQUIRED reconciliation preserves draft",
    "Includes RC10.2 publish truth + RC10.1 photo upload fixes",
    "Upgrade path: codes ≤18 → RC10.3 (code 19)",
  ].join("\n"),
