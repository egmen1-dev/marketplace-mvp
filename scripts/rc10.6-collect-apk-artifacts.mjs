#!/usr/bin/env node
/** Collect APK artifacts for RC10.6 build (PR #198 physical gap closure). */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const ARTIFACT_DIR = resolve("artifacts/closed-beta-rc10.6");
const APK_NAME = "lot_android_closed_beta_0.1.15_beta.7.apk";
...
const manifest = {
  ...
  versionCode: 22,
  commitSha,
  mergeCommitSha: "8d67190",
  pr198MergeSha: "8d67190",
  buildTime,
  environment: "staging",
  ...
  artifact: {
    fileName: APK_NAME,
    path: `artifacts/closed-beta-rc10.6/${APK_NAME}`,
    downloadUrl: `https://raw.githubusercontent.com/egmen1-dev/marketplace-mvp/main/artifacts/closed-beta-rc10.6/${APK_NAME}`,
    sha256,
    sizeBytes,
  },
  android: {
  ...
  embeddedMetadata: {
    ...
    oneTapGuards: true,
    previousRelease: "RC10.5 0.1.15-beta.6 (code 21)",
  },
  buildNotes: {
    release:
      "RC10.6 — physical retest build after PR #198 (photo Continue one-tap + submit black-hole closure + async OCR boundary)",
    previousRelease: "RC10.5 0.1.15-beta.6 (code 21)",
    versionPolicy:
      "Closed Beta RC series increments versionCode monotonically (RC10.5=21 → RC10.6=22) and versionName beta patch (0.1.15-beta.6 → 0.1.15-beta.7)",
  },
};
