#!/usr/bin/env node
/** Publish RC10.1 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc10.1/mrp-publish-report.json");

const manifestPath = resolve("artifacts/closed-beta-rc10.1/build-manifest.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC10.1 — P0 Seller Photo Upload hotfix",
    "Fix: expo-file-system File upload (expo/fetch FormData compatibility)",
    "Fix: upload on photo pick with progress + retry without form reset",
    "Fix: publish guard while upload in flight",
    "Includes RC10 stack: EPIC 158.1–158.3 + EPIC 159",
    "Upgrade path: RC3–RC10 (code 7-16) → RC10.1 (code 17)",
  ].join("\n"),
...
async function main() {
  ...
  const report = {
    ...
    status,
    updatePath: {
      code13: "OPTIONAL_UPDATE→17",
      code14: "OPTIONAL_UPDATE→17",
      code15: "OPTIONAL_UPDATE→17",
      code16: "OPTIONAL_UPDATE→17",
      code17: "NO_UPDATE",
      code18: "NO_UPDATE",
    },
