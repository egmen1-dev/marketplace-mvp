#!/usr/bin/env node
/** RC9 update API verification + published APK SHA256 match. */
import { createHash } from "node:crypto";
import { readFileSync, writeFileSync, mkdirSync } from "node:fs";
import { resolve } from "node:path";

const STAGING = process.env.STAGING_BASE_URL ?? "https://web-production-e56fb.up.railway.app";
const OUT = resolve("artifacts/closed-beta-rc9/update-api-verification.json");
const manifest = JSON.parse(readFileSync(resolve("artifacts/closed-beta-rc9/build-manifest.json"), "utf8"));
...
async function probe(versionCode) {
  ...
  let verdict = "FAIL";
  if ([7, 8, 9, 10, 11, 12, 13].includes(versionCode)) {
    verdict = updateState === "OPTIONAL_UPDATE" && targetCode === 14 ? "PASS" : "FAIL";
  } else if ([14, 15].includes(versionCode)) {
