#!/usr/bin/env node
/** Publish RC7 to MRP via staging admin API. */
...
const OUT = resolve("artifacts/closed-beta-rc7/mrp-publish-report.json");

const manifestPath = resolve("artifacts/closed-beta-rc7/build-manifest.json");
...
const RELEASE = {
  ...
  channel: "BETA",
  releaseNotes: [
    "Closed Beta RC7 — physical validation fixes (PR #148)",
    "Compact CategoryRail — hides empty categories",
    "ProductCard grid layout contract",
    "Cart images via resolveImageUrl",
    "New BootSplash + compact Beta indicator",
    "RC6 preserved: CommerceHeader, chat, badges, update flow",
    "Upgrade path: RC3–RC6 (code 7-11) → RC7 (code 12)",
  ].join("\n"),
...
async function main() {
  ...
  const report = {
    ...
    status,
    updatePath: {
      code7: "OPTIONAL_UPDATE→12",
      code8: "OPTIONAL_UPDATE→12",
      code9: "OPTIONAL_UPDATE→12",
      code10: "OPTIONAL_UPDATE→12",
      code11: "OPTIONAL_UPDATE→12",
      code12: "NO_UPDATE",
      code13: "NO_UPDATE",
    },
