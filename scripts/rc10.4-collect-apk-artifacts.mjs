#!/usr/bin/env node
/** Collect APK artifacts for RC10.4 build (moderation + backend stability). */
import { createHash } from "node:crypto";
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";

const ARTIFACT_DIR = resolve("artifacts/closed-beta-rc10.4");
const APK_NAME = "lot_android_closed_beta_0.1.15_beta.5.apk";
...
const manifest = {
  ...
  artifact: {
    fileName: APK_NAME,
    path: `artifacts/closed-beta-rc10.4/${APK_NAME}`,
    downloadUrl: `https://raw.githubusercontent.com/egmen1-dev/marketplace-mvp/main/artifacts/closed-beta-rc10.4/${APK_NAME}`,
    sha256,
    sizeBytes,
  },
  android: {
  ...
  embeddedMetadata: {
    moderationSellerUx: true,
    p0DynamicCharacteristics: true,
    p0LotPublishTruthFix: true,
    androidInAppUpdater: true,
    backendStabilityFixes: true,
    previousRelease: "RC10.3 0.1.15-beta.4 (code 19)",
  },
  buildNotes: {
    release: "RC10.4 — moderation seller UX + backend stability (catalog phrase fast-path, P1017 reconnect)",
    previousRelease: "RC10.3 0.1.15-beta.4 (code 19)",
  },
};
