...
function resolveGitCommit(): string {
  try {
    ...
    return CLOSED_ALPHA_RELEASE_015.gitCommit;
  }
}
...
/** Publish 0.1.5-alpha P0 hotfix — expo-clipboard + ErrorUtils */
export async function ensureClosedAlpha015ReleasePublished(input: PublishClosedAlphaInput = {}) {
  ...
  if (!row) {
    row = await prisma.mobileReleaseVersion.create({
      data: {
        ...
        minBackendVersion: "mobile-v1",
        minAppVersion: CLOSED_ALPHA_RELEASE_014.versionName,
  ...
  return {
    ...
    previousVersionCode: CLOSED_ALPHA_RELEASE_014.versionCode,
    downloadUrl: CLOSED_ALPHA_APK_DOWNLOAD_URL,
