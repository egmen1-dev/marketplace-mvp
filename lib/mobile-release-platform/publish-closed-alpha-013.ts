...
function resolveGitCommit(): string {
  try {
    ...
    return CLOSED_ALPHA_RELEASE_013.gitCommit;
  }
}
...
async function ensureReleaseHistoryPublished() {
  const seeds = [
    ...
    },
    {
      ...
      downloadUrl: "https://github.com/egmen1-dev/marketplace-mvp/releases/download/closed-alpha-0.1.1/lot-android-alpha-0.1.1.apk",
      rolloutPercent: 0,
...
export async function ensureClosedAlpha013ReleasePublished(input: PublishClosedAlphaInput = {}) {
  ...
  if (!row) {
    row = await prisma.mobileReleaseVersion.create({
      data: {
        ...
        minBackendVersion: "mobile-v1",
        minAppVersion: CLOSED_ALPHA_RELEASE_012.versionName,
  ...
  return {
    ...
    previousVersionCode: CLOSED_ALPHA_RELEASE_012.versionCode,
    downloadUrl: CLOSED_ALPHA_APK_DOWNLOAD_URL,
